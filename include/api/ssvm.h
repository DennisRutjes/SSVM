...
typedef struct SSVM_HostFunctionContext {
...
/// NOT call the `SSVM_StringDelete`.
///
/// \param Buf the buffer to copy into the SSVM_String object.
/// \param Len the buffer length.
///
...
/// released and the object should NOT be used.
///
...
/// function list of registered named modules in the store context, please call
/// `SSVM_StoreFindModule` to get the module instance context and then call
/// `SSVM_ModuleInstanceGetFunctionExports` instead.
/// The returned function names are created and stored in `Names` array, and
/// the caller should call `SSVM_StringDelete` to delete them. If the `Names`
/// and `FuncAddrs` buffer lengths are smaller than the result of the exported
/// functions list size, the overflowed return values will be discarded.
///
/// \param Cxt the SSVM_StoreContext.
...
SSVM_CAPI_EXPORT extern uint32_t
SSVM_StoreGetFunctionExports(const SSVM_StoreContext *Cxt, SSVM_String *Names,
...
/// table list of registered named modules in the store context, please call
/// `SSVM_StoreFindModule` to get the module instance context and then call
/// `SSVM_ModuleInstanceGetTableExports` instead.
/// The returned table names are created and stored in `Names` array, and the
/// caller should call `SSVM_StringDelete` to delete them. If the `Names` and
/// `TableAddrs` buffer lengths are smaller than the result of the exported
/// table list size, the overflowed return values will be discarded.
///
/// \param Cxt the SSVM_StoreContext.
/// \param [out] Names the output names SSVM_String buffer of exported tables.
/// \param [out] TableAddrs the instance addresses buffer of table addresses.
/// \param Len the buffer length.
///
/// \returns actual exported table list size.
SSVM_CAPI_EXPORT extern uint32_t
SSVM_StoreGetTableExports(const SSVM_StoreContext *Cxt, SSVM_String *Names,
...
/// memory list of registered named modules in the store context, please call
/// `SSVM_StoreFindModule` to get the module instance context and then call
/// `SSVM_ModuleInstanceGetMemoryExports` instead.
/// The returned memory names are created and stored in `Names` array, and the
/// caller should call `SSVM_StringDelete` to delete them. If the `Names` and
/// `MemoryAddrs` buffer lengths are smaller than the result of the exported
/// memory list size, the overflowed return values will be discarded.
///
/// \param Cxt the SSVM_StoreContext.
/// \param [out] Names the output names SSVM_String buffer of exported memories.
/// \param [out] MemoryAddrs the instance addresses buffer of memory addresses.
/// \param Len the buffer length.
///
/// \returns actual exported memory list size.
SSVM_CAPI_EXPORT extern uint32_t
SSVM_StoreGetMemoryExports(const SSVM_StoreContext *Cxt, SSVM_String *Names,
...
/// global list of registered named modules in the store context, please call
/// `SSVM_StoreFindModule` to get the module instance context and then call
/// `SSVM_ModuleInstanceGetGlobalExports` instead.
/// The returned global names are created and stored in `Names` array, and the
/// caller should call `SSVM_StringDelete` to delete them. If the `Names`, and
/// `GlobalAddrs` buffer lengths are smaller than the result of the exported
/// global list size, the overflowed return values will be discarded.
///
/// \param Cxt the SSVM_StoreContext.
/// \param [out] Names the output names SSVM_String buffer of exported globals.
/// \param [out] GlobalAddrs the instance addresses buffer of global addresses.
/// \param Len the buffer length.
///
/// \returns actual exported global list size.
SSVM_CAPI_EXPORT extern uint32_t
SSVM_StoreGetGlobalExports(const SSVM_StoreContext *Cxt, SSVM_String *Names,
                           SSVM_InstanceAddress *GlobalAddrs,
                           const uint32_t Len);

/// Get the instantiated anonymous module instance context.
///
/// The module instance context links to the module instance in the store
/// context and owned by the store context.
///
...
/// \returns pointer to the module instance context. NULL if not found.
SSVM_CAPI_EXPORT extern SSVM_ModuleInstanceContext *
SSVM_StoreGetActiveModule(SSVM_StoreContext *Cxt);
...
///
/// \param Cxt the SSVM_StoreContext.
/// \param Name the module name SSVM_String registered in the store context.
///
/// \returns pointer to the module instance context. NULL if not found.
SSVM_CAPI_EXPORT extern SSVM_ModuleInstanceContext *
SSVM_StoreFindModule(SSVM_StoreContext *Cxt, const SSVM_String Name);

/// Get the function type by function name.
///
/// After instantiating a WASM module, the WASM module is registered into the
/// store context as an anonymous module. Then you can call this function to get
/// the exported function's type of the anonymous module by the function name.
/// If you want to get the function's type of registered named modules in the
/// store context, please call `SSVM_StoreFindFunctionTypeRegistered` instead.
...
SSVM_StoreFindFunctionType(SSVM_StoreContext *Cxt,
                           const SSVM_String FuncName);

/// Get the function type by function name.
///
/// After registering a WASM module, you can call this function to get the
/// exported function's type by the module name and the function name.
/// The caller owns the returned function type context, and should call the
/// `SSVM_FunctionTypeDelete` to delete it.
///
/// \param Cxt the SSVM_StoreContext.
...
/// \returns the function type. NULL if the function not found.
SSVM_CAPI_EXPORT extern SSVM_FunctionTypeContext *
SSVM_StoreFindFunctionTypeRegistered(SSVM_StoreContext *Cxt,
...
SSVM_CAPI_EXPORT extern uint32_t SSVM_ModuleInstanceGetFunctionExports(
    const SSVM_ModuleInstanceContext *Cxt, SSVM_String *Names,
...
/// free it. The following is an example to create a host function context.
/// ```c
/// void FuncAdd(SSVM_MemoryInstanceContext *Cxt,
...
/// be `void (SSVM_MemoryInstanceContext *, SSVM_Value *, SSVM_Value *)`. The
/// first `SSVM_Value *` is the input parameters array with length guaranteed
/// to be the same as the parameter types in the `Type`. The second `SSVM_Value
/// *` is the output results array with length guaranteed to be the same as the
/// result types in the `Type`.
...
SSVM_HostFunctionCreate(const SSVM_FunctionTypeContext *Type,
                        void (*HostFunc)(SSVM_MemoryInstanceContext *,
...
SSVM_ImportObjectCreate(const SSVM_String ModuleName);
...
/// NOT access or delete the host function context after calling this function.
///
