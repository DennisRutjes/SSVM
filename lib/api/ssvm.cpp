...
inline std::string genStr(const SSVM_String S) {
...
CONVFROM(HostFunc, Runtime::HostFunctionBase, HostFunction, )
...
inline uint32_t fillMap(const std::map<std::string, uint32_t, std::less<>> &Map,
                        SSVM_String *Names, SSVM_InstanceAddress *FuncAddrs,
                        const uint32_t Len) {
  if (Names && FuncAddrs) {
    uint32_t I = 0;
    for (auto &&Pair : Map) {
...
class CAPIHostFunc : public SSVM::Runtime::HostFunctionBase {
public:
  CAPIHostFunc(const SSVM::Runtime::Instance::FType *Type,
               void (*FuncPtr)(SSVM_MemoryInstanceContext *, const SSVM_Value *,
                               SSVM_Value *),
               const uint64_t FuncCost = 0)
      : SSVM::Runtime::HostFunctionBase(FuncCost), Func(FuncPtr) {
    FuncType = *Type;
  }
  virtual ~CAPIHostFunc() = default;

  SSVM::Expect<void> run(SSVM::Runtime::Instance::MemoryInstance *MemInst,
                         SSVM::Span<const SSVM::ValVariant> Args,
                         SSVM::Span<SSVM::ValVariant> Rets) override {
    std::vector<SSVM_Value> Params(FuncType.Params.size()),
        Returns(FuncType.Returns.size());
    for (uint32_t I = 0; I < Args.size(); I++) {
      Params[I].Value = SSVM::retrieveValue<__int128>(Args[I]);
    }
    SSVM_Value *PPtr = Params.size() ? (&Params[0]) : nullptr;
    SSVM_Value *RPtr = Returns.size() ? (&Returns[0]) : nullptr;
    Func(toMemCxt(MemInst), PPtr, RPtr);
...
private:
  void (*Func)(SSVM_MemoryInstanceContext *, const SSVM_Value *, SSVM_Value *);
};
...
SSVM_Result SSVM_InterpreterRegisterModule(SSVM_InterpreterContext *Cxt,
  ...
  return wrap(
      [&]() {
        return Cxt->Interp.registerModule(
            *fromStoreCxt(StoreCxt), *ASTCxt->Module.get(), genStr(ModuleName));
      },
      EmptyThen, Cxt, StoreCxt, ASTCxt);
}
...
SSVM_Result SSVM_InterpreterInvoke(SSVM_InterpreterContext *Cxt,
  ...
  return wrap(
      [&]() -> SSVM::Expect<std::vector<SSVM::ValVariant>> {
        ...
        const auto FuncIter = FuncExp.find(genStr(FuncName));
        if (FuncIter == FuncExp.cend()) {
          LOG(ERROR) << SSVM::ErrCode::FuncNotFound;
          LOG(ERROR) << SSVM::ErrInfo::InfoExecuting("", genStr(FuncName));
...
SSVM_Result SSVM_InterpreterInvokeRegistered(
    ...
    const uint32_t ReturnLen) {
  auto ParamVec = genValVariantVec(Params, ParamLen);
  auto ModStr = genStr(ModuleName);
  auto FuncStr = genStr(FuncName);
  return wrap(
      [&]() -> SSVM::Expect<std::vector<SSVM::ValVariant>> {
        /// Get module instance.
...
uint32_t SSVM_StoreGetTableExports(const SSVM_StoreContext *Cxt,
                                   SSVM_String *Names,
...
uint32_t SSVM_StoreGetMemoryExports(const SSVM_StoreContext *Cxt,
                                    SSVM_String *Names,
...
uint32_t SSVM_StoreGetGlobalExports(const SSVM_StoreContext *Cxt,
                                    SSVM_String *Names,
...
SSVM_ModuleInstanceContext *SSVM_StoreGetActiveModule(SSVM_StoreContext *Cxt) {
  if (Cxt) {
...
SSVM_ModuleInstanceContext *SSVM_StoreFindModule(SSVM_StoreContext *Cxt,
  ...
  if (Cxt) {
    if (auto Res = fromStoreCxt(Cxt)->findModule(genStr(Name))) {
      return toModCxt(*Res);
    }
  }
  return nullptr;
...
SSVM_StoreFindFunctionType(SSVM_StoreContext *Cxt, const SSVM_String FuncName) {
  if (Cxt) {
    const auto FuncExp = fromStoreCxt(Cxt)->getFuncExports();
    const auto FuncIter = FuncExp.find(genStr(FuncName));
    if (FuncIter != FuncExp.cend()) {
      const auto *FuncInst = *fromStoreCxt(Cxt)->getFunction(FuncIter->second);
      return toFTypeCxt(
...
SSVM_StoreFindFunctionTypeRegistered(SSVM_StoreContext *Cxt,
                                     ...
                                     const SSVM_String FuncName) {
  if (Cxt) {
    if (auto Res = fromStoreCxt(Cxt)->findModule(genStr(ModuleName))) {
      const auto *ModInst = *Res;
      const auto &FuncExp = ModInst->getFuncExports();
      const auto FuncIter = FuncExp.find(genStr(FuncName));
      if (FuncIter != FuncExp.cend()) {
        const auto *FuncInst =
            *fromStoreCxt(Cxt)->getFunction(FuncIter->second);
        return toFTypeCxt(
            new SSVM::Runtime::Instance::FType(FuncInst->getFuncType()));
      }
    }
...
SSVM_ImportObjectCreate(const SSVM_String ModuleName) {
  return toImpObjCxt(new SSVM::Runtime::ImportObject(genStr(ModuleName)));
}
...
void SSVM_ImportObjectAddHostFunction(SSVM_ImportObjectContext *Cxt,
  ...
  if (Cxt && HostFuncCxt) {
    fromImpObjCxt(Cxt)->addHostFunc(
        genStr(Name), std::unique_ptr<SSVM::Runtime::HostFunctionBase>(
                          fromHostFuncCxt(HostFuncCxt)));
  }
}
...
void SSVM_ImportObjectAddTable(SSVM_ImportObjectContext *Cxt,
  ...
  if (Cxt && TableCxt) {
    fromImpObjCxt(Cxt)->addHostTable(
        genStr(Name), std::unique_ptr<SSVM::Runtime::Instance::TableInstance>(
                          fromTabCxt(TableCxt)));
  }
}
...
void SSVM_ImportObjectAddMemory(SSVM_ImportObjectContext *Cxt,
  ...
  if (Cxt && MemoryCxt) {
    fromImpObjCxt(Cxt)->addHostMemory(
        genStr(Name), std::unique_ptr<SSVM::Runtime::Instance::MemoryInstance>(
...
void SSVM_ImportObjectAddGlobal(SSVM_ImportObjectContext *Cxt,
  ...
  if (Cxt && GlobalCxt) {
    fromImpObjCxt(Cxt)->addHostGlobal(
        genStr(Name), std::unique_ptr<SSVM::Runtime::Instance::GlobalInstance>(
...
SSVM_Result SSVM_VMRegisterModuleFromFile(SSVM_VMContext *Cxt,
                                          ...
                                          const char *Path) {
  return wrap(
      [&]() { return Cxt->VM.registerModule(genStr(ModuleName), Path); },
      EmptyThen, Cxt);
}
...
SSVM_Result SSVM_VMRegisterModuleFromBuffer(SSVM_VMContext *Cxt,
  ...
  return wrap(
      [&]() {
        return Cxt->VM.registerModule(genStr(ModuleName), genSpan(Buf, BufLen));
      },
      EmptyThen, Cxt);
}
...
SSVM_VMRegisterModuleFromASTModule(SSVM_VMContext *Cxt,
  ...
  return wrap(
      [&]() {
        return Cxt->VM.registerModule(genStr(ModuleName),
                                      *ASTCxt->Module.get());
      },
...
SSVM_Result SSVM_VMRunWasmFromFile(SSVM_VMContext *Cxt, const char *Path,
  ...
  auto ParamVec = genValVariantVec(Params, ParamLen);
  return wrap(
      [&]() { return Cxt->VM.runWasmFile(Path, genStr(FuncName), ParamVec); },
      [&](auto Res) { fillSSVM_ValueArr(*Res, Returns, ReturnLen); }, Cxt);
}
...
SSVM_VMRunWasmFromBuffer(SSVM_VMContext *Cxt, const uint8_t *Buf,
  ...
  return wrap(
      [&]() {
        return Cxt->VM.runWasmFile(genSpan(Buf, BufLen), genStr(FuncName),
                                   ParamVec);
      },
      [&](auto &&Res) { fillSSVM_ValueArr(*Res, Returns, ReturnLen); }, Cxt);
}
...
SSVM_Result SSVM_VMRunWasmFromASTModule(
  ...
  return wrap(
      [&]() {
        return Cxt->VM.runWasmFile(*ASTCxt->Module.get(), genStr(FuncName),
                                   ParamVec);
      },
      [&](auto &&Res) { fillSSVM_ValueArr(*Res, Returns, ReturnLen); }, Cxt,
      ASTCxt);
...
SSVM_Result SSVM_VMExecute(SSVM_VMContext *Cxt, const SSVM_String FuncName,
                           ...
                           SSVM_Value *Returns, const uint32_t ReturnLen) {
  auto ParamVec = genValVariantVec(Params, ParamLen);
  return wrap([&]() { return Cxt->VM.execute(genStr(FuncName), ParamVec); },
...
SSVM_VMExecuteRegistered(SSVM_VMContext *Cxt, const SSVM_String ModuleName,
  ...
  return wrap(
      [&]() {
        return Cxt->VM.execute(genStr(ModuleName), genStr(FuncName), ParamVec);
      },
      [&](auto &&Res) { fillSSVM_ValueArr(*Res, Returns, ReturnLen); }, Cxt);
}
...
SSVM_VMGetFunctionTypeRegistered(SSVM_VMContext *Cxt,
  ...
  if (Cxt) {
    ...
    if (auto Res = Store.findModule(genStr(ModuleName))) {
      const auto *ModInst = *Res;
      const auto &FuncExp = ModInst->getFuncExports();
      const auto FuncIter = FuncExp.find(genStr(FuncName));
      if (FuncIter != FuncExp.cend()) {
